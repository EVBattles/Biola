// Time Worked
// 10/9/17 6:30 PM - 8:00 PM; 9:20 PM - 10:30 PM
// 10/10/17 10:40 AM - 11:00 AM
// 10/12/17 2:29 PM - 6:00 PM
 ...
 //While it is not the end of the character string stored in the statement array
 {
	...
	// in the components string there is at least one space between two tokens. 
		if	(
				...
				|| 
				((c == '!') && (nextChar != '='))
				// exclamaiton point not followed by an equal sign
				||
				c==',' ||c==':' || c==';' 
...
{
	if ((token == "function") || (token == "read") || (token == "display") || (token == "if") ||
...
// get two kinds of lexical information from 
...

{
	...
	{
		perLineTokenVector name1;
		tokenVectors.push_back(name1);
		getPerLineTokenVectFromOneStringObject(linesOfStatements[i], tokenVectors[i]);
		perLineCategoryVector name2;
		categoryVectors.push_back(name2);
